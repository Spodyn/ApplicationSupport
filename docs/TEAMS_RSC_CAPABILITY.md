...
- bot/agent install scope `team` for standard-channel coverage;
- bot/agent install scope `groupchat` for group-chat coverage;
...
- bot/agent scopes include `team` and `groupchat` for those two installation models;
...
No tenant credentials or production Microsoft 365 credentials are introduced merely to close this capability freeze. When E18-T02 creates the development Teams app/Entra/bot configuration, its first smoke must execute the vendor flow against a non-production sandbox:

- install in a test team; send a standard-channel message without `@mention`; verify one inbound activity;
- install/re-install in a test group chat; send a message without `@mention`; verify one inbound activity;
- do **not** enable private/shared monitoring for v1;
- if private/shared experimentation is performed, record it as non-v1 evidence only and do not change the capability flag without an explicit product scope decision.

This split prevents secrets from being committed during the architecture POC while still making the next environment ticket deterministic.
...
- **E18-T02 / USI-180:** create the real Teams app/Entra/bot development configuration from this contract; wire secrets only through the external secret store; execute the live standard-channel and group-chat smoke.
- **E18-T03 / USI-181:** authenticate/validate inbound Teams activities or notifications and durably ingest them. It must not broaden context scope.
...
Authoritative Microsoft documentation reviewed on 2026-08-29:
