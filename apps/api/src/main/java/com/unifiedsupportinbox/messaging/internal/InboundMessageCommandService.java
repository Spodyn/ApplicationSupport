...
/**
 * Provider-neutral inbound CREATE consumer. Provider adapters normalize root messages and thread
 * replies into the same command; CaseCreationService owns active-vs-terminal generation routing.
 */
@Service
class InboundMessageCommandService implements InboundMessageCommandHandler {

    private final CaseCreationService cases;
    private final CustomerMessageUnreadService unread;
    private final JdbcTemplate jdbc;

    InboundMessageCommandService(
    ...
    private static void validateCreate(Command command) {
        ...
        if (command.mutation() != Mutation.CREATE) {
            throw notReady("Inbound message mutations are handled by later messaging tasks.");
        }
        if (provider != IntegrationProvider.SLACK) {
            throw notReady("Only Slack CREATE mapping is active in the current Slack-first phase.");
        }
    }
